...
def Run(maxLength, batchSize, learningRate, epochs, chainReset, rank, alpha, projectName):
    print("Running RoBERTaBASE adapted to MRPC dataset via LoRA, rank=" + str(rank) +" alpha=" +str(alpha) + " epochs=" +str(epochs) + " lr=" +str(learningRate) + " batchSize=" +str(batchSize))
    ds = load_dataset("glue", "cola")
    ds_two = load_dataset("glue", "mrpc")
    
    print("Printing MRPC START")
    print(ds_two)
    print("Printing MRPC END")
    ...
    run_name = "RoBERTaBase-LoRA-R" + str(lora_r)
    ...
    def train(model, train_loader, epochs, optimizer, chain_reset):
        ...
        for epoch in range(epochs):
            ...
            strName = './RoBERTa/RoBERTaBASE/MRPC/ModelCheckpoints/LoRA-R-' + str(lora_r) + '-Epoch-' + str(epoch) + '.pth'
        ...
        bestLocation = './RoBERTa/RoBERTaBASE/MRPC/ModelCheckpoints/LoRA-R-' + str(lora_r) + '-Epoch-' + str(best_epoch) + '.pth'
        checkpoint = torch.load(bestLocation, map_location='cpu')
        model.load_state_dict(checkpoint)
        bestName = './RoBERTa/RoBERTaBASE/MRPC/ModelCheckpoints/LoRA-R-' + str(lora_r) + '-Best.pth'
    ...
    test_param_count = count_parameters(lora_model)
    print("Model without LoRA post freezing count:", test_param_count)
    ...
    lora_model.linear = LoRALayer(lora_model.linear, lora_r, lora_alpha)
    lora_model.classifier = LoRALayer(lora_model.classifier, lora_r, lora_alpha)
...
if __name__ == "__main__":
    Run(maxLength=288, batchSize=8, learningRate=4e-4, epochs=100, chainReset=0, rank=4, alpha=0, projectName="LoRA_Data_Collection")
