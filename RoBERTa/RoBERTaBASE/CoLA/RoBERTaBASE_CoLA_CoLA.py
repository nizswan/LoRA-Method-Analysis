...
def Run(maxLength, batchSize, learningRate, epochs, chainReset, rank, alpha, projectName):
    print("Running RoBERTaBASE adapted to CoLA dataset via CoLA, rank=" + str(rank) +" alpha=" +str(alpha) + " epochs=" +str(epochs) + " lr=" +str(learningRate) + " batchSize=" +str(batchSize))
    ...
    run_name = "RoBERTaBase-CoLA-CoLA-R" + str(lora_r)
    ...
    def train(model, train_loader, epochs, optimizer, chain_reset):
        ...
        for epoch in range(epochs):
            ...
            strName = './RoBERTa/RoBERTaBASE/CoLA/ModelCheckpoints/CoLA-R' + str(lora_r) + '-Epoch-' + str(epoch) + '.pth'
            strNameTwo = ""
            if (epoch == 0):
                strNameTwo = './RoBERTa/RoBERTaBASE/CoLA/ModelCheckpointsBest/CoLA-R' + str(lora_r) + '-Epoch-' + str(epoch) + '.pth'
        ...
        bestLocation = './RoBERTa/RoBERTaBASE/CoLA/ModelCheckpoints/CoLA-R' + str(lora_r) + '-Epoch-' + str(best_epoch) + '.pth'
        checkpoint = torch.load(bestLocation, map_location='cpu')
        model.load_state_dict(checkpoint)
        bestName = './RoBERTa/RoBERTaBASE/CoLA/ModelCheckpointsBest/CoLA-R' + str(lora_r) + '-Best.pth'
