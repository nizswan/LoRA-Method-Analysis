...
# Added to calculate matthews correlation coefficient
...
def Run(maxLength, batchSize, learningRate, epochs, chainReset, rank, alpha, projectName):
    print("Running RoBERTaBASE adapted to CoLA dataset via FFT epochs=" +str(epochs) + " lr=" +str(learningRate) + " batchSize=" +str(batchSize))
    ...
    def train(model, train_loader, epochs, optimizer, chain_reset):
        ...
        for epoch in range(epochs):
            ...
            strName = './RoBERTa/RoBERTaBASE/CoLA/ModelCheckpoints/FFT-Epoch-' + str(epoch) + '.pth'
            strNameTwo = ""
            if (epoch == 0):
                strNameTwo = './RoBERTa/RoBERTaBASE/CoLA/ModelCheckpointsBest/FFT-Epoch-' + str(epoch) + '.pth'
            ...
            if(val_data[1] - best_val > 0.2 and epoch >= 10):
                print("Reached a validation loss that is at least 0.2 beyond the best validation loss and we've done at least 10 epochs, ending training early")
                break
        ...
        bestLocation = './RoBERTa/RoBERTaBASE/CoLA/ModelCheckpoints/FFT-Epoch-' + str(best_epoch) + '.pth'
        checkpoint = torch.load(bestLocation, map_location='cpu')
        model.load_state_dict(checkpoint)
        bestName = './RoBERTa/RoBERTaBASE/CoLA/ModelCheckpointsBest/FFT-Best.pth'
