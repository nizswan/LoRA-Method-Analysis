...
def Run(maxLength, batchSize, learningRate, epochs, chainReset, rank, alpha, projectName):
    print("Running RoBERTaBASE adapted to MRPC dataset via FFT, epochs=" +str(epochs) + " lr=" +str(learningRate) + " batchSize=" +str(batchSize))
    ...
    class RobertaWithClassification(torch.nn.Module):
        ...
        def forward(self, input_ids, attention_mask):
            output_with_pooling = self.roberta(input_ids=input_ids, attention_mask=attention_mask)
    ...
    model_temp = RobertaWithClassification()
    model_temp.to(device)
    
    def count_parameters(model):
        return sum(p.numel() for p in model.parameters() if p.requires_grad)
    
    base_param_count = count_parameters(model_temp)
    ...
    def train(model, train_loader, epochs, optimizer, chain_reset):
        ...
        for epoch in range(epochs):
            ...
            strName = './RoBERTa/RoBERTaBASE/MRPC/ModelCheckpoints/FFT-Epoch-' + str(epoch) + '.pth'
            strNameTwo = ""
            if (epoch == 0):
                strNameTwo = './RoBERTa/RoBERTaBASE/MRPC/ModelCheckpointsBest/FFT-Epoch-' + str(epoch) + '.pth'
        ...
        bestLocation = './RoBERTa/RoBERTaBASE/MRPC/ModelCheckpoints/FFT-Epoch-' + str(best_epoch) + '.pth'
        checkpoint = torch.load(bestLocation, map_location='cpu')
        model.load_state_dict(checkpoint)
        bestName = './RoBERTa/RoBERTaBASE/MRPC/ModelCheckpointsBest/FFT-Best.pth'
